...
module Passbook
  class Signer
    ...
    def sign(data)
      wwdc  = OpenSSL::X509::Certificate.new file_data(wwdc_cert, :wwdc_cert)
    ...
    def compute_cert
      @key_hash = {}
      if key
        @key_hash[:key]  = OpenSSL::PKey::RSA.new file_data(key, :key), password
        @key_hash[:cert] = OpenSSL::X509::Certificate.new file_data(certificate, :cert)
      else
